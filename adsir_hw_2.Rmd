...
```{r}
#best_lasso <- lending_club_cv %>% 
  #select_best(metric = "accuracy")
#lending_club_lasso_final_wf<- lending_club_lasso_wf %>% 
  #finalize_workflow(best_lasso)

#lending_club_lasso_explain <- 
  #explain_tidymodels(
    #model = lending_club_ranger_final_wf,
    #data = lending_club_train %>% select(-Class), 
    #y = lending_club_train %>%  pull(Class),
    #label = "lasso"
  #)
```
...
```{r}
best_ranger <- lending_club_ranger_cv %>% 
  select_best(metric = "accuracy")
...
lending_club_rf_var_imp <- 
  model_parts(lending_club_rf_explain)
...
```{r}
cp_profile <- function(explainer, new_observation, variable_name) {
  predict_profile(explainer = explainer, new_observation = new_observation) %>% 
  ggplot(aes_string(x = variable_name, y = `_yhat_`)) +
    geom_line()
}
```

```{r}
cp_profile(explainer = lending_club_rf_explain, new_observation = lending_club_train %>% 
             slice_sample(), variable_name = int_rate)
```
...
rf_pdp <- model_profile(explainer = lending_club_rf_explain)

plot(rf_pdp, 
     variables = "funded_amnt",
     geom = "profiles")
```
...
lending_club_stack <- 
  ...
  add_candidates(lending_club_ranger_fit) %>% 
  add_candidates(lending_club_knn_tune)
```

```{r}
as_tibble(lending_club_stack)
```
...
```{r}
autoplot(house_blend, type = "funded_amnt")
